...
export class CrTabBoxElement extends CustomElement {
    ...
    constructor() {
        ...
        this.tabs_.addEventListener('click', (e) => {
    ...
    attributeChangedCallback(name, _oldValue, newValue) {
        ...
        this.getPanels_().forEach((panel, index) => {
            panel.toggleAttribute('selected', index === newIndex);
        });
        this.getTabs_().forEach((tab, index) => {
        ...
        this.dispatchEvent(new CustomEvent('selected-index-change', { bubbles: true, composed: true, detail: newIndex }));
    }
    getTabs_() {
        return Array.from(this.tabs_.querySelector('slot').assignedElements());
    }
    getPanels_() {
    ...
    onKeydown_(e) {
        ...
        const newIndex = (Number(this.getAttribute('selected-index')) + delta + count) % count;
