...
export class CSSShadowEditor extends Common.ObjectWrapper.eventMixin(
  UI.Widget.VBox
) {
  typeField;
  outsetButton;
  ...
  constructor() {
    ...
    this.contentElement.setAttribute(
      "jslog",
      `${VisualLogging.dialog("cssShadowEditor").parent("mapped").track({ keydown: "Enter|Escape" })}`
    );
  ...
  createTextInput(field, propertyName, jslogContext) {
    ...
    textInput.setAttribute(
      "jslog",
      `${VisualLogging.value().track({ change: true, keydown: "ArrowUp|ArrowDown" }).context(jslogContext)}`
    );
    return textInput;
  }
...
export var Events = /* @__PURE__ */ ((Events2) => {
  Events2["SHADOW_CHANGED"] = "ShadowChanged";
  return Events2;
})(Events || {});
