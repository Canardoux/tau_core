...
export class Widget {
  ...
  constructor(useShadowDom, delegatesFocus, element) {
    ...
    this.shadowRoot = this.element.shadowRoot || void 0;
  ...
  static getOrCreateWidget(element) {
    ...
    return new Widget(void 0, void 0, element);
  }
  markAsRoot() {
  ...
  inNotification() {
    return Boolean(this.notificationDepth) || Boolean(this.parentWidgetInternal && this.parentWidgetInternal.inNotification());
  }
  parentIsShowing() {
    if (this.isRoot) {
      return true;
  ...
  focus() {
    ...
    if (this.defaultFocusedChild && this.defaultFocusedChild.visibleInternal) {
      ...
      for (const child2 of this.childrenInternal) {
        if (child2.visibleInternal) {
          child2.focus();
          return;
        }
      }
  ...
  `  *
     ...
     */
  pendingUpdate() {
    return RenderCoordinator.RenderCoordinator.RenderCoordinator.instance().findPendingWrite(this.#id) || Promise.resolve();
  }
}
