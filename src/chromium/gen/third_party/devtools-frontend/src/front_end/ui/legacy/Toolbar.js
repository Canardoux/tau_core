...
export class Toolbar {
  ...
  constructor(className, parentElement) {
    ...
    this.enabled = true;
    this.shadowRoot = createShadowRootWithCoreStyles(this.element, { cssFile: toolbarStyles, delegatesFocus: void 0 });
    ...
    const items = await Promise.all(filtered.map((extension) => {
      ...
      if (actionId) {
        return Toolbar.createActionButtonForId(
          actionId,
          { label, showLabel: Boolean(showLabel), userActionCode: void 0 }
        );
      }
      if (!loadItem) {
...
export class ToolbarComboBox extends ToolbarItem {
  ...
  constructor(changeHandler, title, className, jslogContext) {
    ...
    if (jslogContext) {
      this.selectElementInternal.setAttribute(
        "jslog",
        `${VisualLogging.dropDown().track({ change: true }).context(jslogContext)}`
      );
    }
  }
  selectElement() {
  ...
  createOption(label, value) {
    ...
    const jslogContext = value ? Platform.StringUtilities.toKebabCase(value) : void 0;
