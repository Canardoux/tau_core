...
export class FlameChart extends Common.ObjectWrapper.eventMixin(UI.Widget.VBox) {
  ...
  updateHighlight() {
    ...
    if (entryIndex === -1) {
      ...
      if (groupIndex >= 0 && this.rawTimelineData && this.rawTimelineData.groups && this.rawTimelineData.groups[groupIndex].selectable) {
        this.viewportElement.style.cursor = "pointer";
      } else {
  ...
  draw() {
    ...
    this.dispatchEventToListeners("ChartPlayableStateChange" /* CHART_PLAYABLE_STATE_CHANGED */, wideEntryExists);
    if (!this.#inTrackConfigEditMode) {
      ...
      for (const decoration of decorationsForEvent) {
        switch (decoration.type) {
          case "CANDY" /* CANDY */: {
            ...
            const stripingEndTime = decoration.endAtTime ? Trace.Helpers.Timing.microSecondsToMilliseconds(decoration.endAtTime) : entryStartTime + duration;
...
export var Events = /* @__PURE__ */ ((Events2) => {
  ...
  Events2["CHART_PLAYABLE_STATE_CHANGED"] = "ChartPlayableStateChange";
