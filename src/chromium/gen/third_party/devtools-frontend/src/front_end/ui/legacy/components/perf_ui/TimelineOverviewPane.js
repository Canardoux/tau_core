...
export class TimelineOverviewPane extends Common.ObjectWrapper.eventMixin(
  ...
  onMouseMove(event) {
    ...
    if (Trace.Types.Timing.MilliSeconds(timeWindow.rawStartValue) <= timeInMilliSeconds && timeInMilliSeconds <= Trace.Types.Timing.MilliSeconds(timeWindow.rawEndValue)) {
  ...
  onWindowChanged(event) {
    ...
    this.windowStartTime = event.data.rawStartValue === this.overviewCalculator.minimumBoundary() ? 0 : event.data.rawStartValue;
    this.windowEndTime = event.data.rawEndValue === this.overviewCalculator.maximumBoundary() ? Infinity : event.data.rawEndValue;
    const windowTimes = {
      startTime: Trace.Types.Timing.MilliSeconds(this.windowStartTime),
      endTime: Trace.Types.Timing.MilliSeconds(this.windowEndTime)
    };
...
export class OverviewInfo {
  ...
  constructor(anchor) {
    ...
    this.visible = false;
    this.element = UI.UIUtils.createShadowRootWithCoreStyles(this.glassPane.contentElement, {
