...
export class DataGridImpl extends Common.ObjectWrapper.ObjectWrapper {
  ...
  announceSelectedGridNode() {
    if (this.element === Platform.DOMUtilities.deepActiveElement(this.element.ownerDocument) && this.selectedNode && this.selectedNode.existingElement()) {
      let expandText;
  ...
  startEditingColumnOfDataGridNode(node, cellIndex) {
    ...
    if (column.dataType === "Boolean" /* BOOLEAN */) {
      const checkboxLabel = UI.UIUtils.CheckboxLabel.create(void 0, node.data[column.id]);
...
export class DataGridNode {
  ...
  constructor(data, hasChildren) {
    ...
    this.cellAccessibleTextMap = /* @__PURE__ */ new Map();
  ...
  createElement() {
    ...
    this.elementInternal.setAttribute(
      "jslog",
      `${VisualLogging.tableRow().track({ keydown: "ArrowUp|ArrowDown|ArrowLeft|ArrowRight|Enter|Space" })}`
    );
...
export class DataGridWidgetElement extends UI.Widget.WidgetElement {
  ...
  createWidget() {
    ...
    dataGridImpl.addEventListener("SelectedNode" /* SELECTED_NODE */, this.#selectedNode.bind(this));
    dataGridImpl.addEventListener("DeselectedNode" /* DESELECTED_NODE */, this.#deselectedNode.bind(this));
    dataGridImpl.addEventListener("OpenedNode" /* OPENED_NODE */, this.#openedNode.bind(this));
    dataGridImpl.addEventListener("SortingChanged" /* SORTING_CHANGED */, this.#sortingChanged.bind(this));
    dataGridImpl.addEventListener("PaddingChanged" /* PADDING_CHANGED */, this.#paddingChanged.bind(this));
