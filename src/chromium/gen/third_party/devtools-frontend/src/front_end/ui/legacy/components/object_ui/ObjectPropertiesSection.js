...
export class ObjectPropertiesSection extends UI.TreeOutline.TreeOutlineInShadow {
  ...
  static valueElementForFunctionDescription(description, includePreview, defaultName) {
    ...
    function nameAndArguments(contents) {
      ...
      if (startOfArgumentsIndex !== -1 && endOfArgumentsMatch && endOfArgumentsMatch.index !== void 0 && endOfArgumentsMatch.index > startOfArgumentsIndex) {
    ...
    function addElements(prefix, body, abbreviation) {
      ...
      if (includePreview) {
        UI.UIUtils.createTextChild(
          valueElement,
          Platform.StringUtilities.trimEndWithMaxLength(body.trim(), maxFunctionBodyLength)
        );
      } else {
  ...
  static createPropertyValue(value, wasThrown, showPreview, parentElement, linkifier, isSyntheticProperty = false, variableName) {
    ...
    function createNodeElement() {
      ...
      valueElement.addEventListener(
        "mousemove",
        () => SDK.OverlayModel.OverlayModel.highlightObjectAsDOMNode(value),
        false
      );
...
let maxRenderableStringLength = 1e4;
...
export class RootElement extends UI.TreeOutline.TreeElement {
  ...
  targetObject;
  toggleOnClick;
  constructor(object, linkifier, emptyPlaceholder, propertiesMode = 1 /* OWN_AND_INTERNAL_AND_INHERITED */, extraProperties = [], targetObject = object) {
...
export class ObjectPropertyTreeElement extends UI.TreeOutline.TreeElement {
  ...
  static async populate(treeElement, value, skipProto, skipGettersAndSetters, linkifier, emptyPlaceholder, propertiesMode = 1 /* OWN_AND_INTERNAL_AND_INHERITED */, extraProperties, targetValue) {
    ...
    switch (propertiesMode) {
      ...
      case 1 /* OWN_AND_INTERNAL_AND_INHERITED */:
        ({ properties, internalProperties } = await SDK.RemoteObject.RemoteObject.loadFromObjectPerProto(
          value,
          true
          /* generatePreview */
        ));
  ...
  update() {
    ...
    if (isInternalEntries) {
      ...
      element.addEventListener("click", (event) => {
        ...
        void object.callFunction(invokeGetter, [SDK.RemoteObject.RemoteObject.toCallArgument(getter)]).then(this.onInvokeGetterClick.bind(this));
      }, false);
    } else {
      this.valueElement = document.createElement("span");
  ...
  updatePropertyPath() {
    ...
    const parentPath = this.parent instanceof ObjectPropertyTreeElement && this.parent.nameElement && !this.parent.property.synthetic ? this.parent.nameElement.title : "";
      ...
      if (ranges.length === 1) {
        ...
        for (let i = 0; i < ranges.length; ++i) {
          ...
          if (fromIndex2 === toIndex2) {
            await ArrayGroupingTreeElement.populateAsFragment(treeNode, object, fromIndex2, toIndex2, linkifier);
          } else {
            treeNode.appendChild(new ArrayGroupingTreeElement(object, fromIndex2, toIndex2, count, linkifier));
          }
        }
      }
      if (topLevel) {
...
export class ExpandableTextPropertyValue extends ObjectPropertyValue {
  ...
  appendApplicableItems(_event, contextMenu, _object) {
    if (this.text.length < this.maxDisplayableTextLength && this.expandElement) {
      contextMenu.clipboardSection().appendItem(
        this.expandElementText || "",
        this.expandText.bind(this),
  ...
  expandText() {
    ...
    if (this.expandElement.parentElement) {
      this.expandElement.parentElement.insertBefore(
        document.createTextNode(this.text.slice(this.maxLength)),
        this.expandElement
      );
    }
