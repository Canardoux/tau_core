...
export class TextPrompt extends Common.ObjectWrapper.ObjectWrapper {
  ...
  acceptSuggestionOnStopCharacters(key) {
    if (!this.currentSuggestion || !this.queryRange || key.length !== 1 || !this.completionStopCharacters || !this.completionStopCharacters.includes(key)) {
      return false;
    }
  ...
  isSuggestBoxVisible() {
    return this.suggestBox !== void 0 && this.suggestBox.visible();
  }
  isCaretInsidePrompt() {
    const selection = this.element().getComponentSelection();
