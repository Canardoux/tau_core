...
class BaseLinkSwatch extends HTMLElement {
  ...
  set data(data) {
    ...
    data.showTitle = data.showTitle === void 0 ? true : data.showTitle;
    this.render(data);
  }
  get linkElement() {
  ...
  render(data) {
    ...
    const { startNode } = render(
      html`<button .disabled=${!isDefined} class=${classes}
                     title=${ifDefined(data.showTitle ? title : void 0)}
                     data-title=${ifDefined(!data.showTitle ? title : void 0)}
...
export class LinkSwatch extends HTMLElement {
  ...
  render(data) {
    ...
    render(
      html`<span title=${data.text} jslog=${VisualLogging.link().track({ click: true }).context(jslogContext)}><devtools-base-link-swatch .data=${{
        text,
        isDefined,
        title,
        onLinkActivate
      }}></devtools-base-link-swatch></span>`,
      this.shadow,
      { host: this }
    );
  }
