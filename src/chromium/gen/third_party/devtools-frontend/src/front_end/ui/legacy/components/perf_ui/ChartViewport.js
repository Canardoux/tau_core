...
export class ChartViewport extends UI.Widget.VBox {
  ...
  constructor(delegate, config) {
    ...
    UI.UIUtils.installDragHandle(
      ...
      this.endRangeSelection.bind(this),
      "text",
      null
    );
  ...
  onMouseWheel(wheelEvent) {
    ...
    if (navigation === "classic") {
      ...
      } else if (wheelEvent.ctrlKey || Math.abs(wheelEvent.deltaX) > Math.abs(wheelEvent.deltaY)) {
        this.handleZoomGesture(zoomDelta);
      } else {
        this.vScrollElement.scrollTop += scrollDelta;
      }
  ...
  handlePanGesture(offset, animate) {
    ...
    const timeOffset = Platform.NumberUtilities.clamp(
      this.pixelToTimeOffset(offset),
      this.minimumBoundary - bounds.left,
