...
export class Spectrum extends Common.ObjectWrapper.eventMixin(UI.Widget.VBox) {
    ...
    for (let i = 0; i < palette.colors.length; i++) {
      ...
      colorElement.addEventListener(
        ...
        this.paletteColorSelected.bind(
          this,
          palette.colors[i],
          palette.colorNames[i],
          Boolean(palette.matchUserFormat)
        )
      );
      colorElement.addEventListener(
        "focus",
  ...
  paletteColorSelected(colorText, colorName, matchUserFormat) {
    ...
    this.innerSetColor(
      ...
      colorText,
      colorName,
      matchUserFormat ? this.colorFormat : color.format(),
      ChangeSource.Other
    );
  }
  ...
  setColor(color) {
    this.innerSetColor(color, "", void 0, color.format(), ChangeSource.Model);
    const colorValues = color.as(Common.Color.Format.HSL).canonicalHSLA();
    UI.ARIAUtils.setValueNow(this.hueElement, colorValues[0]);
    UI.ARIAUtils.setValueText(this.alphaElement, colorValues[3]);
  }
  ...
  showSrgbOverlay() {
    if (this.contrastDetails && this.contrastDetails.expanded() || this.gamut !== "display-p3" /* DISPLAY_P3 */) {
      return;
    }
  ...
  async showFormatPicker(event) {
    ...
    await contextMenu.show(event, (newColor) => {
      this.innerSetColor(newColor, void 0, void 0, newColor.format(), ChangeSource.Other);
    });
  ...
  inputChanged(event) {
    ...
    this.innerSetColor(color, void 0, void 0, colorFormat, ChangeSource.Input);
  }
  wasShown() {
  ...
  async toggleColorPicker(enabled) {
    ...
    if (!IS_NATIVE_EYE_DROPPER_AVAILABLE) {
      ...
      try {
        ...
        this.innerSetColor(color ?? void 0, "", void 0, void 0, ChangeSource.Other);
      } catch (error) {
