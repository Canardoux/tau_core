...
const nonDomDebugElements = /* @__PURE__ */ new WeakMap();
...
function processImpressionsForIntuitiveDebugLog(states) {
  ...
  for (const state of states) {
    ...
    if (!state.parent || !impressions.has(state.parent?.veid)) {
      ...
      const parent2 = impressions.get(state.parent?.veid);
  ...
  setTimeout(() => {
    if (pendingEventExpectation?.missingEvents) {
      pendingEventExpectation.fail(new Error(
        "Missing VE Events: " + pendingEventExpectation.missingEvents.map((e) => "interaction" in e ? e.interaction : formatImpressions(e.impressions)).join("\n")
      ));
    }
