...
export class UISourceCode extends Common.ObjectWrapper.ObjectWrapper {
  ...
  contentCommitted(content, committedByUser) {
    this.lastAcceptedContent = null;
    this.contentInternal = new TextUtils.ContentData.ContentData(content, Boolean(this.contentEncodedInternal), this.mimeType());
    this.requestContentPromise = null;
  ...
  workingCopyContentData() {
    ...
    const contentData = this.contentInternal ? TextUtils.ContentData.ContentData.contentDataOrEmpty(this.contentInternal) : TextUtils.ContentData.EMPTY_TEXT_CONTENT_DATA;
    if (this.workingCopyInternal !== null) {
      return new TextUtils.ContentData.ContentData(
        this.workingCopyInternal,
        /* isBase64 */
        false,
  ...
  workingCopyChanged() {
    ...
    this.projectInternal.workspace().dispatchEventToListeners(
      WorkspaceImplEvents.WorkingCopyChanged,
      { uiSourceCode: this }
    );
  }
