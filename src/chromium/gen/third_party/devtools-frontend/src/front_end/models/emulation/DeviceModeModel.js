...
export class DeviceModeModel extends Common.ObjectWrapper.ObjectWrapper {
  ...
  constructor() {
    ...
    this.#appliedUserAgentTypeInternal = "Desktop" /* DESKTOP */;
    ...
    this.#uaSettingInternal = Common.Settings.Settings.instance().createSetting("emulation.device-ua", "Mobile" /* MOBILE */);
    this.#uaSettingInternal.addChangeListener(this.uaSettingChanged, this);
    this.#deviceScaleFactorSettingInternal = Common.Settings.Settings.instance().createSetting("emulation.device-scale-factor", 0);
    this.#deviceScaleFactorSettingInternal.addChangeListener(this.deviceScaleFactorSettingChanged, this);
    this.#deviceOutlineSettingInternal = Common.Settings.Settings.instance().moduleSetting("emulation.show-device-outline");
  ...
  outlineImage() {
    return this.#deviceInternal && this.#modeInternal && this.#deviceOutlineSettingInternal.get() ? this.#deviceInternal.outlineImage(this.#modeInternal) : "";
  }
  outlineRect() {
  ...
  isMobile() {
    switch (this.#typeInternal) {
      ...
      case "Responsive" /* Responsive */:
        return this.#uaSettingInternal.get() === "Mobile" /* MOBILE */ || this.#uaSettingInternal.get() === "Mobile (no touch)" /* MOBILE_NO_TOUCH */;
    }
    return false;
  }
  enabledSetting() {
  ...
  reset() {
    ...
    this.setWidth(400);
    this.setHeight(0);
    this.#uaSettingInternal.set("Mobile" /* MOBILE */);
  }
  ...
  currentOutline() {
    ...
    if (this.#typeInternal !== "Device" /* Device */ || !this.#deviceInternal || !this.#modeInternal) {
      return outline;
    }
  ...
  currentInsets() {
    if (this.#typeInternal !== "Device" /* Device */ || !this.#modeInternal) {
  ...
  calculateAndEmulate(resetPageScaleFactor) {
    ...
    if (this.#typeInternal === "Device" /* Device */ && this.#deviceInternal && this.#modeInternal) {
      ...
      if (mobile) {
        this.#appliedUserAgentTypeInternal = this.#deviceInternal.touch() ? "Mobile" /* MOBILE */ : "Mobile (no touch)" /* MOBILE_NO_TOUCH */;
      } else {
        this.#appliedUserAgentTypeInternal = this.#deviceInternal.touch() ? "Desktop (touch)" /* DESKTOP_TOUCH */ : "Desktop" /* DESKTOP */;
      }
      this.applyDeviceMetrics(
        new UI.Geometry.Size(orientation.width, orientation.height),
        insets,
        outline,
        this.#scaleSettingInternal.get(),
        this.#deviceInternal.deviceScaleFactor,
        mobile,
    ...
    } else if (this.#typeInternal === "None" /* None */) {
    ...
    } else if (this.#typeInternal === "Responsive" /* Responsive */) {
      ...
      this.applyDeviceMetrics(
        new UI.Geometry.Size(screenWidth, screenHeight),
        new Insets(0, 0, 0, 0),
        new Insets(0, 0, 0, 0),
        this.#scaleSettingInternal.get(),
        this.#deviceScaleFactorSettingInternal.get() || defaultDeviceScaleFactor,
        mobile,
      ...
      this.applyTouch(
        this.#uaSettingInternal.get() === "Desktop (touch)" /* DESKTOP_TOUCH */ || this.#uaSettingInternal.get() === "Mobile" /* MOBILE */,
        this.#uaSettingInternal.get() === "Mobile" /* MOBILE */
      );
    }
    if (overlayModel) {
      overlayModel.setShowViewportSizeOnResize(this.#typeInternal === "None" /* None */);
    }
  ...
  applyDeviceMetrics(screenSize, insets, outline, scale, deviceScaleFactor, mobile, screenOrientation, resetPageScaleFactor, forceMetricsOverride = false) {
    ...
    this.#outlineRectInternal = new Rect(
      this.#screenRectInternal.left - outline.left * scale,
      0,
    ...
    if (!forceMetricsOverride) {
      ...
      if (this.#visiblePageRectInternal.width === pageWidth * scale && this.#visiblePageRectInternal.height === pageHeight * scale && Number.isInteger(pageWidth * scale) && Number.isInteger(pageHeight * scale)) {
        pageWidth = 0;
        pageHeight = 0;
      }
    }
  ...
  showHingeIfApplicable(overlayModel) {
    const orientation = this.#deviceInternal && this.#modeInternal ? this.#deviceInternal.orientationByName(this.#modeInternal.orientation) : null;
