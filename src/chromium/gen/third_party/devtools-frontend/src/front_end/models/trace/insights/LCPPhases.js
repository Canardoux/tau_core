...
function breakdownPhases(nav, docRequest, lcpMs, lcpRequest) {
  ...
  const firstDocByteTs = Helpers.Timing.secondsToMicroseconds(docReqTiming.requestTime) + Helpers.Timing.millisecondsToMicroseconds(docReqTiming.receiveHeadersStart);
...
export function generateInsight(parsedTrace, context) {
  ...
  const lcpTs = metricScore.event?.ts ? Helpers.Timing.microSecondsToMilliseconds(metricScore.event?.ts) : void 0;
  ...
  if (!lcpRequest) {
    return finalize({
      ...
      lcpEvent,
      lcpRequest,
      phases: breakdownPhases(context.navigation, docRequest, lcpMs, lcpRequest) ?? void 0
    });
  }
  return finalize({
    lcpMs,
