...
export class PresentationConsoleMessageManager {
  ...
  consoleMessageAdded(consoleMessage) {
    ...
    if (!consoleMessage.isErrorOrWarning() || !consoleMessage.runtimeModel() || consoleMessage.source === Protocol.Log.LogEntrySource.Violation || !runtimeModel) {
      return;
    }
    const level = consoleMessage.level === Protocol.Log.LogEntryLevel.Error ? Workspace.UISourceCode.Message.Level.ERROR : Workspace.UISourceCode.Message.Level.WARNING;
    this.#sourceFrameMessageManager.addMessage(
      new Workspace.UISourceCode.Message(level, consoleMessage.messageText),
      consoleMessage,
      runtimeModel.target()
    );
  }
...
export class PresentationSourceFrameMessageHelper {
  ...
  constructor() {
    ...
    Workspace.Workspace.WorkspaceImpl.instance().addEventListener(
      ...
      this.#uiSourceCodeAdded.bind(this)
    );
  }
...
export class PresentationSourceFrameMessage {
  ...
  async #updateLocation(liveLocation) {
    ...
    this.#message.range = TextUtils.TextRange.TextRange.createFromLocation(uiLocation.lineNumber, uiLocation.columnNumber || 0);
