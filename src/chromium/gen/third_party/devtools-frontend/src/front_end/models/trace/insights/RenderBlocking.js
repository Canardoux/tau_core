...
function hasImageLCP(parsedTrace, context) {
  return parsedTrace.LargestImagePaint.lcpRequestByNavigation.get(context.navigation) !== void 0;
}
...
export function generateInsight(parsedTrace, context) {
  ...
  const firstPaintTs = parsedTrace.PageLoadMetrics.metricScoresByFrameId.get(context.frameId)?.get(context.navigationId)?.get(Handlers.ModelHandlers.PageLoadMetrics.MetricName.FP)?.event?.ts;
  if (!firstPaintTs) {
    return finalize({
      renderBlockingRequests: [],
