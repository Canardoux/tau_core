...
export class DebuggerWorkspaceBinding {
  ...
  static instance(opts = { forceNew: null, resourceMapping: null, targetManager: null }) {
    ...
    if (!debuggerWorkspaceBindingInstance || forceNew) {
      if (!resourceMapping || !targetManager) {
        throw new Error(
          `Unable to create DebuggerWorkspaceBinding: resourceMapping and targetManager must be provided: ${new Error().stack}`
        );
      }
  ...
  async rawLocationToUILocation(rawLocation) {
    for (const sourceMapping of this.#sourceMappings) {
      const uiLocation2 = sourceMapping.rawLocationToUILocation(rawLocation);
      if (uiLocation2) {
        return uiLocation2;
      }
    }
  ...
  async uiLocationToRawLocations(uiSourceCode, lineNumber, columnNumber) {
    for (const sourceMapping of this.#sourceMappings) {
      const locations2 = sourceMapping.uiLocationToRawLocations(uiSourceCode, lineNumber, columnNumber);
  ...
  async uiLocationRangeToRawLocationRanges(uiSourceCode, textRange) {
    for (const sourceMapping of this.#sourceMappings) {
      const ranges2 = sourceMapping.uiLocationRangeToRawLocationRanges(uiSourceCode, textRange);
      if (ranges2) {
        return ranges2;
      }
    }
