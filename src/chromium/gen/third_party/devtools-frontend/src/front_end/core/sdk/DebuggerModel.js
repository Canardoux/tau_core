...
export class DebuggerModel extends SDKModel {
  ...
  constructor(target) {
    ...
    Common.Settings.Settings.instance().moduleSetting("js-source-maps-enabled").addChangeListener((event) => this.#sourceMapManagerInternal.setEnabled(event.data));
  ...
  asyncStackTracesStateChanged() {
    ...
    const enabled = !Common.Settings.Settings.instance().moduleSetting("disable-async-stack-traces").get() && this.#debuggerEnabledInternal;
...
export class Scope {
  ...
  extraProperties() {
    if (this.#ordinal !== 0 || this.#typeInternal !== Protocol.Debugger.ScopeType.Local || this.#callFrameInternal.script.isWasm()) {
      return [];
    }
