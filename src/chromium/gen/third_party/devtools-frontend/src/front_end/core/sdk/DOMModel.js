...
export class DOMNode {
  ...
  init(doc, isInShadowTree, payload) {
    ...
    if (payload.templateContent) {
      this.templateContentInternal = DOMNode.create(this.#domModelInternal, this.ownerDocument, true, payload.templateContent);
    ...
    if (payload.importedDocument) {
      this.#importedDocumentInternal = DOMNode.create(this.#domModelInternal, this.ownerDocument, true, payload.importedDocument);
  ...
  removeChild(node) {
    ...
    if (node.#subtreeMarkerCount) {
      this.#domModelInternal.dispatchEventToListeners("MarkersChanged" /* MarkersChanged */, this);
    }
    this.renumber();
  }
  ...
  setMarker(name, value) {
    if (value === null) {
      ...
      for (let node = this; node; node = node.parentNode) {
        this.#domModelInternal.dispatchEventToListeners("MarkersChanged" /* MarkersChanged */, node);
      }
      return;
    }
...
export class DOMModel extends SDKModel {
  ...
  scrollableFlagUpdated(nodeId, isScrollable) {
    ...
    this.dispatchEventToListeners("ScrollableFlagUpdated" /* ScrollableFlagUpdated */, { node });
  }
...
export var Events = /* @__PURE__ */ ((Events2) => {
  ...
  Events2["ScrollableFlagUpdated"] = "ScrollableFlagUpdated";
  return Events2;
})(Events || {});
class DOMDispatcher {
  #domModel;
