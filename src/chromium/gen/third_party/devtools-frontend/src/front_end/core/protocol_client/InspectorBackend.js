...
export class SessionRouter {
  ...
  sendMessage(sessionId, domain, method, params, callback) {
    ...
    if (test.onMessageSent) {
      ...
      test.onMessageSent(
        { domain, method, params: paramsObject, id: messageId, sessionId },
  ...
  prepareParameters(method, parameters, args, errorCallback) {
    ...
    for (const param of parameters) {
      ...
      if (!args.length && !optionalFlag) {
        errorCallback(
          `Protocol Error: Invalid number of arguments for method '${method}' call. It must have the following arguments ${JSON.stringify(parameters)}'.`
        );
        return null;
      }
      ...
      if (typeof value !== expectedJSType) {
        errorCallback(
          `Protocol Error: Invalid type of argument '${paramName}' for method '${method}' call. It must be '${typeName}' but it is '${typeof value}'.`
        );
        return null;
      }
