...
export class ThrottlingManager {
  ...
  createNetworkThrottlingSelector(selectElement) {
    ...
    selectElement.setAttribute(
      "jslog",
      `${VisualLogging.dropDown().track({ change: true }).context(this.currentNetworkThrottlingConditionsSetting.name)}`
    );
    ...
    function populate(groups) {
      ...
      for (let i = 0; i < groups.length; ++i) {
        ...
        for (const conditions of group.items) {
          ...
          const jslogContext = i === groups.length - 1 ? "custom-network-throttling-item" : Platform.StringUtilities.toKebabCase(conditions.i18nTitleKey || title);
  ...
  createCPUThrottlingSelector() {
    const control = new UI.Toolbar.ToolbarComboBox(
      (event) => this.setCPUThrottlingRate(this.cpuThrottlingRates[event.target.selectedIndex]),
  ...
  createHardwareConcurrencySelector() {
    ...
    checkbox.checkboxElement.disabled = true;
