...
export class CSSOverviewCompletedView extends UI.Widget.VBox {
  ...
  constructor(controller) {
    ...
    this.#splitWidget = new UI.SplitWidget.SplitWidget(true, false, void 0, 200);
    ...
    this.#elementContainer.addEventListener("TabClosed" /* TAB_CLOSED */, (evt) => {
      if (evt.data === 0) {
...
export class DetailsView extends Common.ObjectWrapper.eventMixin(UI.Widget.VBox) {
  ...
  constructor() {
    ...
    this.#tabbedPane.addEventListener(UI.TabbedPane.Events.TabClosed, () => {
      this.dispatchEventToListeners("TabClosed" /* TAB_CLOSED */, this.#tabbedPane.tabIds().length);
    });
  }
...
export class ElementDetailsView extends UI.Widget.Widget {
  ...
  constructor(controller, domModel, cssModel, linkifier) {
    ...
    this.#elementGrid.addEventListener(
      DataGrid.DataGrid.Events.SORTING_CHANGED,
      this.#sortMediaQueryDataGrid.bind(this)
    );
