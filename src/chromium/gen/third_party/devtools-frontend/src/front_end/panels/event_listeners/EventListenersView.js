...
export class EventListenersView extends UI.Widget.VBox {
  ...
  constructor(changeCallback, enableDefaultTreeFocus = false) {
    ...
    this.treeItemMap = /* @__PURE__ */ new Map();
  }
  focus() {
  ...
  addObject(object) {
    ...
    async function markInternalEventListeners() {
      ...
      return frameworkEventListenersObject.internalHandlers.object().callFunctionJSON(isInternalEventListener, eventListeners.map(handlerArgument)).then(setIsInternal);
