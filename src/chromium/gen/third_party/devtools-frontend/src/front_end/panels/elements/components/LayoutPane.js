    ...
    await coordinator.write("LayoutPane render", () => {
      render(html`
        <details open>
          ...
          ${gridElements ? html`<div class="content-section" jslog=${VisualLogging.section("grid-overlays")}>
              <h3 class="content-section-title">
                ${gridElements.length ? i18nString(UIStrings.gridOverlays) : i18nString(UIStrings.noGridLayoutsFoundOnThisPage)}
              </h3>
              ${gridElements.length ? html`<div class="elements">
                  ${gridElements.map((element) => this.#renderElement(element))}
                </div>` : ""}
            </div>` : ""}
        </details>
        ${flexContainerElements !== void 0 ? html`
          <details open>
            ...
            ${flexContainerElements ? html`<div class="content-section" jslog=${VisualLogging.section("flexbox-overlays")}>
                <h3 class="content-section-title">
                  ${flexContainerElements.length ? i18nString(UIStrings.flexboxOverlays) : i18nString(UIStrings.noFlexboxLayoutsFoundOnThisPage)}
                </h3>
                ${flexContainerElements.length ? html`<div class="elements">
  ...
  #renderEnumSetting(setting) {
    ...
    return html`<label data-enum-setting="true" class="select-label" title=${setting.title}>
      <select
        ...
        ${setting.options.map(
      (opt) => html`<option value=${opt.value} .selected=${setting.value === opt.value} jslog=${VisualLogging.item(Platform.StringUtilities.toKebabCase(opt.value)).track({ click: true })}>${opt.title}</option>`
    )}
      </select>
    </label>`;
  }
