...
export class SearchView extends UI.Widget.VBox {
  ...
  constructor(settingKey, throttler) {
    ...
    this.search.setAttribute(
      "jslog",
      `${VisualLogging.textField().track({ change: true, keydown: "ArrowUp|ArrowDown|Enter" })}`
    );
    ...
    const refreshButton = new UI.Toolbar.ToolbarButton(i18nString(UIStrings.refresh), "refresh", void 0, "search.refresh");
    const clearButton = new UI.Toolbar.ToolbarButton(i18nString(UIStrings.clear), "clear", void 0, "search.clear");
  ...
  buildSearchConfig() {
    return new Workspace.SearchConfig.SearchConfig(
      this.search.value,
      !this.matchCaseButton.toggled,
      this.regexButton.toggled
    );
  }
