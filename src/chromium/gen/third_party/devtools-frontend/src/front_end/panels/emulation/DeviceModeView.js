...
export class DeviceModeView extends UI.Widget.VBox {
  ...
  constructor() {
    ...
    this.mediaInspector = new MediaQueryInspector(
      () => this.model.appliedDeviceSize().width,
      this.model.setWidth.bind(this.model),
  ...
  populatePresetsContainer() {
    ...
    for (let i = sizes.length - 1; i >= 0; --i) {
      ...
      block.setAttribute(
        "jslog",
        `${VisualLogging.action().track({ click: true }).context(`device-mode-preset-${sizes[i]}px`)}`
      );
  ...
  onResizeUpdate(widthFactor, heightFactor, event) {
    ...
    if (widthFactor && this.resizeStart) {
      ...
      if (newWidth >= EmulationModel.DeviceModeModel.MinDeviceSize && newWidth <= EmulationModel.DeviceModeModel.MaxDeviceSize) {
    ...
    if (heightFactor && this.resizeStart) {
      ...
      if (newHeight >= EmulationModel.DeviceModeModel.MinDeviceSize && newHeight <= EmulationModel.DeviceModeModel.MaxDeviceSize) {
