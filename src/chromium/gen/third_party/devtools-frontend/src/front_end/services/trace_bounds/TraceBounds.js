...
export class BoundsManager extends EventTarget {
  ...
  setTimelineVisibleWindow(newWindow, options = {
    ...
    if (!options.ignoreMiniMapBounds) {
      newWindow.min = Trace.Types.Timing.MicroSeconds(Math.max(this.#currentState.minimapTraceBounds.min, newWindow.min));
      newWindow.max = Trace.Types.Timing.MicroSeconds(Math.min(this.#currentState.minimapTraceBounds.max, newWindow.max));
    }
    if (newWindow.min === existingWindow.min && newWindow.max === existingWindow.max) {
      return;
    }
