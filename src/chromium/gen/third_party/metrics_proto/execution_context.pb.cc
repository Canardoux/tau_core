...
static const ::PROTOBUF_NAMESPACE_ID::internal::EnumEntry Thread_entries[] = {
  ...
  { {Thread_names + 43, 19}, 29 },
  { {Thread_names + 62, 17}, 11 },
  { {Thread_names + 79, 29}, 14 },
  { {Thread_names + 108, 19}, 25 },
  { {Thread_names + 127, 9}, 7 },
  { {Thread_names + 136, 23}, 18 },
  { {Thread_names + 159, 25}, 28 },
  { {Thread_names + 184, 10}, 22 },
  { {Thread_names + 194, 11}, 2 },
  { {Thread_names + 205, 25}, 3 },
  { {Thread_names + 230, 15}, 8 },
  { {Thread_names + 245, 17}, 17 },
  { {Thread_names + 262, 9}, 6 },
  { {Thread_names + 271, 11}, 1 },
  { {Thread_names + 282, 12}, 20 },
  { {Thread_names + 294, 19}, 19 },
  { {Thread_names + 313, 12}, 12 },
  { {Thread_names + 325, 23}, 4 },
  { {Thread_names + 348, 13}, 9 },
  { {Thread_names + 361, 23}, 13 },
  { {Thread_names + 384, 21}, 15 },
  { {Thread_names + 405, 21}, 26 },
  { {Thread_names + 426, 18}, 16 },
  { {Thread_names + 444, 14}, 0 },
  { {Thread_names + 458, 14}, 10 },
  { {Thread_names + 472, 29}, 27 },
  { {Thread_names + 501, 13}, 21 },
};
...
static const int Thread_entries_by_number[] = {
  ...
  16, // 1 -> MAIN_THREAD
  11, // 2 -> FILE_THREAD
  12, // 3 -> FILE_USER_BLOCKING_THREAD
  20, // 4 -> PROCESS_LAUNCHER_THREAD
  2, // 5 -> CACHE_THREAD
  15, // 6 -> IO_THREAD
  7, // 7 -> DB_THREAD
  13, // 8 -> GPU_MAIN_THREAD
  21, // 9 -> RENDER_THREAD
  27, // 10 -> UTILITY_THREAD
  4, // 11 -> COMPOSITOR_THREAD
  19, // 12 -> OTHER_THREAD
  22, // 13 -> SCHEDULER_WORKER_THREAD
  5, // 14 -> COMPOSITOR_TILE_WORKER_THREAD
  23, // 15 -> SERVICE_WORKER_THREAD
  25, // 16 -> THREAD_POOL_THREAD
  14, // 17 -> GPU_MEMORY_THREAD
  8, // 18 -> DEDICATED_WORKER_THREAD
  18, // 19 -> MEMORY_INFRA_THREAD
  17, // 20 -> MEDIA_THREAD
  29, // 21 -> WEBRTC_THREAD
  10, // 22 -> DRM_THREAD
  0, // 23 -> AUDIO_DEVICE_THREAD
  1, // 24 -> AUDIO_THREAD
  6, // 25 -> DAV1D_WORKER_THREAD
  24, // 26 -> STACK_SAMPLING_THREAD
  28, // 27 -> VIDEO_FRAME_COMPOSITOR_THREAD
  9, // 28 -> DISPLAY_COMPOSITOR_THREAD
  3, // 29 -> CODEC_WORKER_THREAD
};
...
bool Thread_Parse(
  ...
  int int_value;
  bool success = ::PROTOBUF_NAMESPACE_ID::internal::LookUpEnumValue(
      Thread_entries, 30, name, &int_value);
  if (success) {
